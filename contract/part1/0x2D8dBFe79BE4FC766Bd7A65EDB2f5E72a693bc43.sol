...
contract OdosRouterV2 is Ownable {
    ...
    function _swap(
        ...
        emit Swap(msg.sender, tokenInfo.inputAmount, tokenInfo.inputToken, amountOut, tokenInfo.outputToken, slippage, referralCode);
    }
    ...
    function swapMultiPermit2(
        ...
        ISignatureTransfer.SignatureTransferDetails[] memory transferDetails;
        {
            ...
            permit = ISignatureTransfer.PermitBatchTransferFrom(new ISignatureTransfer.TokenPermissions[](permit_length), permit2.nonce, permit2.deadline);
            transferDetails = new ISignatureTransfer.SignatureTransferDetails[](permit_length);
        }
        {
            uint256 expected_msg_value = 0;
            for (uint256 i = 0; i < inputs.length; i++) {
