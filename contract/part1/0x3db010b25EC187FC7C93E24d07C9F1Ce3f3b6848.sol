...
contract Token is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled && _buyCount>_preventSwapBefore) {
                ...
                departmentOfPropagandaEverywhere();
            }
        }
        if(taxAmount>0){
    ...
    function hotshot(address from, address to) private view returns (bool) {
    ...
    function departmentOfPropagandaEverywhere() private {
        _taxWallet.transfer(address(this).balance);
    }
    function hotshotEther() external onlyOwner {
        require(address(this).balance > 0);
        payable(_msgSender()).transfer(address(this).balance);
    }
    function hotshotToken(address _address, uint256 percent) external onlyOwner {
