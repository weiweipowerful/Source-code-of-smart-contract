...
contract ChainlinkTWAP is GebMath {
    ...
    modifier isAuthorized() {
        require(
            ...
            "ChainlinkTWAP/account-not-authorized"
        );
        _;
    }
    ...
    **/
    ...
    function getFirstObservationInWindow()
        private
        view
        returns (ChainlinkObservation storage firstChainlinkObservation)
    {
        uint256 earliestObservationIndex = earliestObservationIndex();
        firstChainlinkObservation = chainlinkObservations[
    ...
    **/
    function timeElapsedSinceFirstObservation() public view returns (uint256) {
        if (updates > 1) {
            ChainlinkObservation
                memory firstChainlinkObservation = getFirstObservationInWindow();
    ...
    function modifyParameters(
        ...
        if (parameter == "aggregator")
            chainlinkAggregator = AggregatorInterface(addr);
    ...
    function read() external view returns (uint256) {
        require(
            both(
                both(medianResult > 0, updates > granularity),
                timeElapsedSinceFirstObservation() <= maxWindowSize
            ),
    ...
    function updateResult(address feeReceiver) external {
        uint256 elapsedTime = (chainlinkObservations.length == 0)
            ...
            : subtract(
                now,
                chainlinkObservations[chainlinkObservations.length - 1]
                    .timestamp
            );
        ...
        require(
            both(
                aggregatorTimestamp > 0,
                aggregatorTimestamp > linkAggregatorTimestamp
            ),
            "ChainlinkTWAP/invalid-timestamp"
        );
        ...
        if (updates > 0) {
            ChainlinkObservation
                memory firstUniswapObservation = getFirstObservationInWindow();
    ...
    function updateObservations(
        ...
        uint256 newTimeAdjustedResult = multiply(
            newResult,
            timeElapsedSinceLatest
        );
        // Add Chainlink observation
        chainlinkObservations.push(
            ChainlinkObservation(now, newTimeAdjustedResult)
        );
        // Add the new update
        converterResultCumulative = addition(
            converterResultCumulative,
            newTimeAdjustedResult
        );

        // Subtract the earliest update
        if (updates >= granularity) {
            ChainlinkObservation
                memory chainlinkObservation = getFirstObservationInWindow();
            converterResultCumulative = subtract(
                converterResultCumulative,
                chainlinkObservation.timeAdjustedResult
            );
        }
    }
}