...
contract cvxFxnStaking is ERC20, ReentrancyGuard{
    ...
    constructor(address _proxy, address _cvxfxn, address _depositor) ERC20(
            "Staked CvxFxn",
            "stkCvxFxn"
        ){
        vefxnProxy = _proxy;
        ...
        IERC20(fxn).approve(_depositor,type(uint256).max);
    }
    ...
    function addReward(
        ...
        require(_rewardsToken != cvxfxn && _rewardsToken != address(this), "invalid token");
    ...
    function deposit(uint256 _amount, bool _lock) public nonReentrant{
        ...
        //deposit, cvxfxn will be returned here
        IFxnDepositor(fxnDepositor).deposit(_amount,_lock);
    ...
    function stake(uint256 _amount) public nonReentrant{
        ...
        //pull cvxfxn
        IERC20(cvxfxn).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function stakeAll() external{
        uint256 balance = IERC20(cvxfxn).balanceOf(msg.sender);
        stake(balance);
    }

    //deposit cvxfxn and accredit a different address
    ...
    function withdraw(uint256 _amount) external nonReentrant{
        ...
        //send cvxfxn
        IERC20(cvxfxn).safeTransfer(msg.sender, _amount);
    ...
    function recoverERC20(address _tokenAddress, uint256 _tokenAmount) external nonReentrant onlyOwner {
        ...
        require(_tokenAddress != cvxfxn, "Cannot withdraw staking token");
