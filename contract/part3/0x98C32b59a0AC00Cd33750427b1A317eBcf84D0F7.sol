...
contract SigMasterChef is Ownable {
    ...
    uint256 public rewardTimeFactor = 4;
    ...
    function totalRewardAtBlock(uint256 atBlock) public view returns (uint256) {
        ...
        uint256 t = (atBlock - startBlock).div(rewardTimeFactor);
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            // 🐧 we use different reward function, so instead the original code:
    ...
    // copy of pendingSushi with explicit blockNumber for the frontend usage
    function pendingSushi2(uint256 _pid, address _user, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber >= block.number, "require blockNumber >= block.number");
        // better name: currPoolInfo
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (blockNumber > pool.lastRewardBlock && lpSupply != 0) {
            uint256 totalSushiReward = totalRewardAtBlock(blockNumber).sub(totalRewardAtBlock(pool.lastRewardBlock));
            uint256 totalSushiRewardForPool = totalSushiReward.mul(pool.allocPoint).div(totalAllocPoint);
            accSushiPerShare = accSushiPerShare.add(totalSushiRewardForPool.mul(1e12).div(lpSupply));
        }
        uint256 userAmountBeforeTax = user.amount.mul(accSushiPerShare).div(1e12).sub(user.rewardDebt);
