...
// File: contracts/TheCryptAvatar.sol
...
contract TheCryptAvatar is Ownable, ERC721A, PaymentSplitter, CreatorTokenBase {
    ...
    uint256 public MAX_TOTAL_PHASE = 5666;

    uint256 public MAX_PER_WALLET_FREE = 1;
    uint256 public MAX_PER_WALLET_WL = 6666;
    uint256 public MAX_PER_WALLET_OG = 6666;
    uint256 public MAX_PER_WALLET_PUBLIC = 6666;

    uint256 public freeSalePrice = 0 ether;
    uint256 public ogSalePrice = 0.019 ether;
    uint256 public wlSalePrice = 0.029 ether;
    uint256 public publicSalePrice = 0.039 ether;

    bytes32 public merkleRootFREE;
    bytes32 public merkleRootOG;
    bytes32 public merkleRootWL;

    mapping(address => uint256) public amountNFTsperWalletFREE;
    ...
    ) CreatorTokenBase() ERC721A("TheCrypt", "CRYPT") PaymentSplitter(_team, _teamShares) Ownable(_initialOwner) {
        baseURI = _baseURI;
    ...
    function whitelistMint(
        ...
        require(
            sellingStep == Step.Active,
            "Whitelist sale is not activated"
        );
        require(msg.sender == _account, "Mint with your own wallet.");
        require(
            _whitelist == 1 || _whitelist == 2 || _whitelist == 3,
            "Whitelist incorrect"
        );
        require(
            totalSupply() + _quantity <= MAX_TOTAL_PHASE,
            "Max supply exceeded"
        );
        ...
        if (_whitelist == 3) {
            ...
            require(isOGListed(msg.sender, _proof), "Not whitelisted");
            require(
                amountNFTsperWalletOG[msg.sender] + _quantity <=
                    MAX_PER_WALLET_OG,
                "Max per wallet limit reached"
            );
            require(msg.value >= ogSalePrice * _quantity, "Not enought funds");
            amountNFTsperWalletOG[msg.sender] += _quantity;
        } else if (_whitelist == 1){
            require(isFREEListed(msg.sender, _proof), "Not whitelisted");
            require(
    ...
    function publicSaleMint(address _account, uint256 _quantity)
        ...
        payable
    {
        require(sellingStep == Step.Active, "Public sale is not activated");
        require(
            totalSupply() + _quantity <= MAX_TOTAL_PHASE,
            "Max supply exceeded"
        );
    ...
    function setMaxTotalPHASE(uint256 _MAX_TOTAL_PHASE) external onlyOwner {
        MAX_TOTAL_PHASE = _MAX_TOTAL_PHASE;
    }

    function setMaxPerWalletFREE(uint256 _MAX_PER_WALLET_FREE)
        external
        onlyOwner
    {
    ...
    function setFREESalePrice(uint256 _freeSalePrice) external onlyOwner {
    ...
    function isFREEListed(address _account, bytes32[] calldata _proof)
        internal
        view
        returns (bool)
    {
        return _verifyFREE(leaf(_account), _proof);
    }
    ...
    function _verifyFREE(bytes32 _leaf, bytes32[] memory _proof)
        ...
        returns (bool)
    {
        return MerkleProof.verify(_proof, merkleRootFREE, _leaf);
    }
