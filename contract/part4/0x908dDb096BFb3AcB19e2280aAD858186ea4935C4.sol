...
contract ESE is ERC20Permit, ERC20Burnable {
    ...
    ///@dev Is address a beneficiary of any vesting stage.
    mapping(address => bool) private _isVestingBeneficiary;
    ...
    function addVestingBeneficiaries(uint256 stage, Beneficiary[] calldata beneficiaries) external onlyBeforeInitialization {
        ...
        for(uint256 i; i < beneficiaries.length;){
            ...
            _isVestingBeneficiary[beneficiary] = true;
            ...
            if(TGEMint != 0) _mint(beneficiary, TGEMint);
    ...
    function totalSupply() public view override returns (uint256) {
        unchecked{
            return super.totalSupply() + _totalReleasableAmount();
        }
    }
    ...
    function _releasableAmount(address account) internal view returns(uint256 amount){
        if(!_isVestingBeneficiary[account]) return 0;
        unchecked{
    ...
    function _totalVestedAmount(VestingParams storage vesting) internal view returns (uint256) {
        return _vestedAmount(vesting, vesting.amount);
    }
    ...
    function _vestedAmount(VestingParams storage vesting, address account) internal view returns (uint256) {
        return _vestedAmount(vesting, vesting.amounts[account]);
    }

    function _vestedAmount(VestingParams storage vesting, uint256 amount) internal view returns (uint256) {
