...
/// @dev Enforces a claim protection delay: broadcasts can only occur at least after the configured
/// delay from the start of the current weekly period.
contract L1Sender is Ownable {
    ...
    /// @notice The claim protection delay in seconds after period start
    uint256 public claimProtectionDelay = 2 hours;
    ...
    /// @notice Emitted when the claim protection delay is changed
    event ClaimProtectionDelayChanged(uint256 oldDelay, uint256 newDelay);
    ...
    /// @dev Enforces that the broadcast only occurs after the claim protection period.
    function broadcastBlock(uint256 chainId, uint256 additionalGasLimit) external payable {
        // Enforce claim protection delay: broadcasts are only allowed after the configured delay from period start
        if (block.timestamp < currentPeriod() + claimProtectionDelay) revert TooSoon();
    ...
    /// @notice Sets the claim protection delay.
    /// @param _claimProtectionDelay The new claim protection delay in seconds.
    /// @dev Only callable by the owner.
    function setClaimProtectionDelay(uint256 _claimProtectionDelay) external onlyOwner {
        uint256 oldDelay = claimProtectionDelay;
        claimProtectionDelay = _claimProtectionDelay;
        emit ClaimProtectionDelayChanged(oldDelay, _claimProtectionDelay);
    }
