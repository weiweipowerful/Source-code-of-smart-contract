...
contract LightChainStakingV1 is Ownable {
    IERC20 public stakingToken;
    using SafeERC20 for IERC20;

    uint256 public rewardRate3Months = 2; // 2% Reward
    uint256 public rewardRate6Months = 5; // 5% Reward
    uint256 public rewardRate12Months = 12; // 12% Reward
    uint256 public rewardRate2Years = 30; // 30% Reward
    uint256 public rewardRate3Years = 70; // 70% Reward

    uint256 public constant MAX_REWARD_RATE = 150; // Max 150% for any duration
    uint256 public constant MIN_REWARD_RATE_3_MONTHS = 1; // Min 1% for 3 Months Stake
    uint256 public constant MIN_REWARD_RATE_6_MONTHS = 3; // Min 3% for 6 Months Stake
    uint256 public constant MIN_REWARD_RATE_12_MONTHS = 9; // Min 9% for 12 Months Stake
    uint256 public constant MIN_REWARD_RATE_24_MONTHS = 20; // Min 20% for 24 Months Stake
    uint256 public constant MIN_REWARD_RATE_36_MONTHS = 30; // Min 30% for 36 Months Stake
    bool public stakingEnabled = true;
    bool public withdrawalsEnabled = true;

    uint256 public earlyWithdrawalPenaltyRate = 40;
    ...
    modifier stakingAllowed() {
        require(stakingEnabled, "LightChain: Staking is disabled");
        _;
    }

    modifier withdrawalsAllowed() {
        require(withdrawalsEnabled, "LightChain: Withdrawals are disabled");
        _;
    }

    function stakeLCAI(uint256 _amount, uint256 _lockPeriod) external stakingAllowed {
        require(_amount > 0, "LightChain: Cannot stake 0 tokens");
        require(
            _lockPeriod == 90 days ||
            _lockPeriod == 180 days ||
            _lockPeriod == 365 days ||
            _lockPeriod == 730 days ||
            _lockPeriod == 1095 days,
            "LightChain: Invalid lock period"
        );
    ...
    function withdraw(uint256 _stakeIndex) external withdrawalsAllowed {
        require(_stakeIndex < stakes[msg.sender].length, "LightChain: Invalid stake index");
        Stake storage userStake = stakes[msg.sender][_stakeIndex];
        require(!userStake.withdrawn, "LightChain: Stake already withdrawn");
        ...
        if (block.timestamp < userStake.endTime) {
            uint256 penalty = (amountToWithdraw * earlyWithdrawalPenaltyRate) /
                100;
    ...
    function reinvestReward(uint256 _stakeIndex) external stakingAllowed {
        ...
        Stake storage userStake = stakes[msg.sender][_stakeIndex];
        require(!userStake.withdrawn, "LightChain: Stake already withdrawn");
        require(block.timestamp >= userStake.endTime, "LightChain: Stake is still locked");

        uint256 reward = calculateReward(msg.sender, _stakeIndex);
        require(reward > 0, "LightChain: No rewards available to reinvest");
    ...
    function getRewardRate(uint256 _lockPeriod) public view returns (uint256) {
        ...
        if (_lockPeriod == 365 days) return rewardRate12Months;
        if (_lockPeriod == 730 days) return rewardRate2Years;
        if (_lockPeriod == 1095 days) return rewardRate3Years;
        revert("LightChain: Invalid lock period");
    }
