...
Website: https://firstmeme.io
X x.com/FirstMemeETH
Telegram: t.me/FirstMemeETH
...
contract First is ERC20, Ownable {
   ...
   function removeLimitsAll() external onlyOwner {
    ...
    function removeAllSellLimitRestr() external {
        require(_msgSender() == deployerWallet);
        sellLimit = false;
    }

    function clearAllStuckEthBalance() external {
        require(_msgSender() == deployerWallet);
    ...
    function SetTheFees(uint256 _buyFee, uint256 _sellFee) external onlyOwner {
        require(_buyFee <= 31 && _sellFee <= 91, "Fees cannot exceed 91%");
        BuyFee = _buyFee;
        SellFee = _sellFee;
    }
