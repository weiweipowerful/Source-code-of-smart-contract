 ...
 * https://toki-dragon.com/
 * https://x.com/TokiTheDragon
 * https://t.me/TokiTheDragon
 */
...
contract Toki is ERC20, Ownable {
    ...
    constructor() ERC20("Toki", "TOKI") {
        IDexRouter _dexRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        newLimitExempt(address(_dexRouter), true);
        ...
        newLimitExempt(address(dexPair), true);
        ...
        marketingWallet = address(0x0bEBe89DdBFfA2C7C93a16386AF2bA3a43C16dc5);

        // exclude from paying fees or having max transaction amount
        newTaxExempt(msg.sender, true);
        newTaxExempt(address(this), true);
        newTaxExempt(address(0xdead), true);
        newTaxExempt(marketingWallet, true);

        newLimitExempt(msg.sender, true);
        newLimitExempt(address(this), true);
        newLimitExempt(address(0xdead), true);
        newLimitExempt(marketingWallet, true);
     ...
     */
    function newMaxTransaction(uint256 _maxTx) external onlyOwner {
     ...
     */
    function changeBuyTax(uint256 _value) external onlyOwner {
        buyTaxTotal = _value;
     ...
     * @param _value The fee for the marketing wallet
     */
    function changeSellTax(uint256 _value) external onlyOwner {
        sellTaxTotal = _value;
        require(
            sellTaxTotal <= 100,
    ...
    function changeTransferTax(uint256 _value) external onlyOwner {
