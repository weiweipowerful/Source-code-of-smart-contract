...
contract LNQFarm is Context, FarmOwner, ReentrancyGuard {
    ...
    uint256[4] public rewardTokensPerBlock = [8, 3, 0, 0]; // Reward tokens per block for each pool
    uint256[4] public lockTime = [180, 90, 60, 30]; // Lock time for each pool

    mapping(uint256 => mapping(address => PoolStaker)) public poolStakers;

    // Events
    event Deposit(address indexed user, uint256 indexed poolId, uint256 amount);
    event Compound(address indexed user, uint256 indexed poolId, uint256 amount);
    ...
    /**
     * @dev Update the reward tokens per block for a specific pool.
     * @param _poolId The ID of the pool to update.
     * @param _newRate The new reward tokens per block rate.
     */
    function updateRewardTokensPerBlock(uint256 _poolId, uint256 _newRate) external onlyOwner {
        require(_poolId >= 0 && _poolId < rewardTokensPerBlock.length, "Pool does not exist");
        uint256 currentTotalRate = 0;
        for (uint256 i = 0; i < rewardTokensPerBlock.length; i++) {
            currentTotalRate += rewardTokensPerBlock[i];
        }
        // Subtract the current rate of the pool being updated
        currentTotalRate -= rewardTokensPerBlock[_poolId];
        require(currentTotalRate + _newRate <= 20, "Total reward tokens per block cannot exceed 20");
        rewardTokensPerBlock[_poolId] = _newRate;
    }
    ...
    function deposit(uint256 _poolId, uint256 _amount) external nonReentrant {
        ...
        if(staker.amount > 0) {
            ...
            // stakerAddresses[_poolId].push(msg.sender); // Add staker address if not already present
        }
    ...
    function compound(uint256 _poolId) external nonReentrant {
        ...
        emit Compound(msg.sender, _poolId, rewards);
    }
     ...
     */
    function getFarmsLiquidity() external view returns(uint256[4] memory) {
        uint256[4] memory farmsLiquidity;
        for(uint8 i = 0; i < 4; i++) {
     ...
     */
    function getAPRForUser(address _user) external view returns(uint256[4] memory) {
        uint256[4] memory APR;
        for(uint8 i = 0; i < 4; i++) {
            Pool storage pool = pools[i];
     ...
     */
    function getStakedToken(address _user) external view returns(uint256[4] memory) {
        uint256[4] memory stakedToken;
        for(uint8 i = 0; i < 4; i++) {
            PoolStaker storage staker = poolStakers[i][_user];
     ...
     */
    function getEarnedToken(address _user) external view returns(uint256[4] memory) {
        uint256[4] memory earnedToken;
        for(uint8 i = 0; i < 4; i++) {
            PoolStaker storage staker = poolStakers[i][_user];
     ...
     */
    function getClaimableRewards(address _user) external view returns(uint256[4] memory) {
        uint256[4] memory claimableRewards;
        for(uint8 i = 0; i < 4; i++) {
     ...
     */
    function checkLockTime(address _user) external view returns(bool[4] memory) {
        bool[4] memory checkLockState;
        for(uint8 i = 0; i < 4; i++) {
            PoolStaker storage staker = poolStakers[i][_user];
            if (staker.endTime == 0) {
