...
// File: contracts/nove/NovaToken.sol
...
contract NovaToken is ERC20, Ownable {
    ...
    constructor(address _router, address _devAddress, address _vaultAddress, address recipient) public ERC20("Nova Token", "NOVA") {
        ...
        _mint(recipient, 1000000000000000 * (10**18)); 
    }
    ...
    function _checkPrice() private {
        ...
        (uint256 cR0, uint256 cR1, ) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        if(cR0 == 0 || cR1 == 0) {
            return;
        }
        if(reserve0 == 0 || reserve1 == 0) {
            ...
            if(isToken0) {
                // price increase 10 times, destroy 1%
                // cR1 / cR0  >= ( reserve1 / reserve0 ) * 10
                if( cR1 * reserve0 >= reserve1 * cR0 * 10 ) {
                    super._transfer(uniswapV2Pair, DEAD, cR0 / 100);
                    IUniswapV2Pair(uniswapV2Pair).sync();
                     (reserve0, reserve1, ) = IUniswapV2Pair(uniswapV2Pair).getReserves();
                }
            } else {
                // price increase 10 times, destroy 1%
                // cR0 / cR1 >= ( reserve0 / reserve1 ) * 10
                if( cR0 * reserve1 >= reserve0 * cR1 * 10 ) {
                    super._transfer(uniswapV2Pair, DEAD, cR1 / 100);
                    IUniswapV2Pair(uniswapV2Pair).sync();
                   (reserve0, reserve1, ) = IUniswapV2Pair(uniswapV2Pair).getReserves();
                }
            }
    ...
    function _transfer(
        ...
        if( !swapping && automatedMarketMakerPairs[to]) {
            ...
            // if(address(this) == IUniswapV2Pair(uniswapV2Pair).token0() && IERC20(WETH).balanceOf(uniswapV2Pair) != _reserve1){
            //     takeFee = false;
            // }
            // if(address(this) == IUniswapV2Pair(uniswapV2Pair).token1() && IERC20(WETH).balanceOf(uniswapV2Pair) != _reserve0){
            //     takeFee = false;
            // }
        }
    ...
    function processFee(uint256 contractTokenBalance) private {
        ...
        uint256 convertAmount = contractTokenBalance;
        uint256 ethBal = address(this).balance;
        swapTokensForEth(convertAmount.mul(75).div(90));
        uint256 ethOut = address(this).balance - ethBal;
        // 3 : 3 : 3 [dev, vault,]
        // 75 share
        // 3% vault 
        // 3% market
        // 1.5% (1/2 3%)  liquidity
        payable(vaultAddress).transfer(ethOut.mul(30).div(75));
        payable(devAddress).transfer(ethOut.mul(30).div(75));
        addLiquidity();
    }
