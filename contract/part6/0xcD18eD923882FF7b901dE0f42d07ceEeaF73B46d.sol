...
contract REX is ERC20, Ownable {
    ...
    constructor() ERC20("REX", "REX") payable {
        uint256 totalSupply = 1234567890 * 1e18;
        uint256 lpSupply = totalSupply.mul(75).div(100);
        uint256 caSupply = totalSupply.mul(22).div(100);

        maxSwapSize = totalSupply.mul(195).div(10000);
        maxWalletAmount = totalSupply.mul(195).div(10000);
        feeThreshold = totalSupply.mul(1).div(100);

        feeRecipient = 0x12f497395ad0e690A671EE41231c01cabE5dbabe;
    ...
    function setTokenFees(uint256 newBuyFee, uint256 newSellFee) external onlyOwner {
        require(newBuyFee <= 20 && newSellFee <= 25, 'Attempting to set fee higher than initial fee.');
