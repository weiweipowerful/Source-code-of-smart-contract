...
// File: contracts/GME.sol
pragma solidity 0.8.25;

contract GameStop is Context, IERC20Metadata, Ownable, ReentrancyGuard {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (!List[sender] && !List[recipient]) {
            // Buy operatiom
            if (sender == Pair && _buyFee > 0) {
                amountToCharge = amount * _buyFee / 100;
            // Sell operation
            } else if (recipient == Pair && _sellFee > 0) {
                amountToCharge = amount * _sellFee / 100;
            }
