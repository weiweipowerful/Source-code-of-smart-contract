...
contract AuthenticallyCryptoNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
 ...
 constructor(
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= MAX_MINT_PER_TX, "Exceeds max mint amount");
    require(balanceOf(msg.sender) + _mintAmount <= MAX_PER_WALLET, "Exceeds max per wallet");
  ...
  function ownerMint(address[] calldata _to) public onlyOwner {
    uint256 supply = totalSupply();
    require(supply + _to.length <= maxSupply, "Exceeds max supply");
