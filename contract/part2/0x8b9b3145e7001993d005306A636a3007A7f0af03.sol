...
contract Hotshot is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != address(this) && to != address(this)) {
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled ) {
                ...
                swapHotshotTokensForEth(min(amount, min(contractTokenBalance, _taxSwap)));
                sendHotshotETHToFee(address(this).balance);
            }
        }
        if( taxAmount> 0){
    ...
    function sendHotshotETHToFee(uint256 amount) private {
        _taxWallet.transfer(amount);
    }

    function swapHotshotTokensForEth(
        uint256 tokenAmount
    ) private lockTheSwap {
        address[] memory path = new address[](2);
        path[0] = address(this);
